...
how it works
Pins live in a Notion database and are fetched at runtime via a Vercel serverless function (/api/pins). The Notion token is stored as a Vercel environment variable — never in the code. The map uses Leaflet.js with CARTO Positron tiles. No frameworks, no build step.
index.html    — map UI
...
Fork the repo
Create a Notion integration and connect it to your database
Add NOTION_API_KEY as a Vercel environment variable
